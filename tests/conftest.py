...
# Import dotz modules
from dotz import core, cli, templates, watcher
...
def initialized_dotz_repo(mock_home):
    ...
    config_data = {
        "file_patterns": {
            "include": [".*", "*.conf", "*.config", "*.cfg", "*.ini", "*.toml", "*.yaml", "*.yml", "*.json"],
            "exclude": [".DS_Store", ".Trash*", ".cache", ".git", ".svn", "*.log", "*.tmp", "*.lock"]
        },
        "search_settings": {
            "recursive": True,
...
def sample_profile_data():
    ...
    return {
        ...
        "environment": {
            "GIT_AUTHOR_EMAIL": "work@company.com",
            "EDITOR": "code"
        },
        "metadata": {
            "created": "2024-01-01T00:00:00Z",
...
def validate_no_side_effects(isolated_home):
    ...
    assert not (real_home / ".dotz").exists() or (real_home / ".dotz").stat().st_mtime < time.time() - 60