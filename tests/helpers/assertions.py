...
def assert_directory_exists(dir_path: Union[str, Path], message: str = "") -> None:
    ...
    assert (
        path.is_dir()
    ), f"Path exists but is not a directory: {path} {message}".strip()


def assert_symlink_correct(
    symlink_path: Union[str, Path], target_path: Union[str, Path], message: str = ""
) -> None:
    """
...
def assert_file_content(
    file_path: Union[str, Path], expected_content: str, message: str = ""
) -> None:
    """
...
def assert_file_contains(
    file_path: Union[str, Path], substring: str, message: str = ""
) -> None:
    """
...
def assert_json_file_valid(
    file_path: Union[str, Path], message: str = ""
) -> Dict[str, Any]:
    """
...
def assert_json_content(
    file_path: Union[str, Path], expected_data: Dict[str, Any], message: str = ""
) -> None:
    """
...
def assert_git_committed(
    repo_path: Union[str, Path], commit_message: str = None, message: str = ""
) -> None:
    """
    ...
    if commit_message:
        ...
        assert (
            commit_message in actual_message
...
def assert_git_clean(repo_path: Union[str, Path], message: str = "") -> None:
    ...
    ), f"Repository has untracked files: {repo.untracked_files} {message}".strip()
...
def assert_config_valid(config_data: Dict[str, Any], message: str = "") -> None:
    ...
    for key in required_keys:
        assert (
            key in config_data
        ), f"Missing required config key: {key} {message}".strip()

    # Check file_patterns structure
    file_patterns = config_data["file_patterns"]
    assert (
        "include" in file_patterns
    ), f"Missing 'include' in file_patterns {message}".strip()
    assert (
        "exclude" in file_patterns
    ), f"Missing 'exclude' in file_patterns {message}".strip()
    assert isinstance(
        file_patterns["include"], list
    ), f"'include' must be a list {message}".strip()
    assert isinstance(
        file_patterns["exclude"], list
    ), f"'exclude' must be a list {message}".strip()
    ...
    for key in required_search_keys:
        assert (
            key in search_settings
        ), f"Missing '{key}' in search_settings {message}".strip()
        assert isinstance(
            search_settings[key], bool
        ), f"'{key}' must be boolean {message}".strip()


def assert_backup_valid(
    backup_path: Union[str, Path], expected_files: List[str] = None, message: str = ""
) -> None:
    """
    ...
    if expected_files:
        for expected_file in expected_files:
            assert (
                expected_file in archive_files
            ), f"Expected file '{expected_file}' not found in backup {path} {message}".strip()


def assert_template_valid(
    template_dir: Union[str, Path], expected_files: List[str] = None, message: str = ""
) -> None:
    """
    ...
    for key in required_metadata_keys:
        assert (
            key in metadata
        ), f"Missing '{key}' in template metadata {message}".strip()
...
def assert_operation_result(
    result: Dict[str, Any], expected_success: bool = True, message: str = ""
) -> None:
    """
    ...
    if expected_success:
        ...
        assert (
            "message" in result or "files_added" in result or "commit_hash" in result
        ...
        if "error" in result:
            assert isinstance(
                result["error"], str
            ), f"Error should be a string {message}".strip()


def assert_files_equal(
    file1: Union[str, Path], file2: Union[str, Path], message: str = ""
) -> None:
    """
...
def assert_list_contains_all(
    actual_list: List[Any], expected_items: List[Any], message: str = ""
) -> None:
    """
...
def assert_list_contains_none(
    actual_list: List[Any], unexpected_items: List[Any], message: str = ""
) -> None:
    """
