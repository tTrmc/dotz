...
class MockConsole:
    ...
    def assert_message_printed(self, expected_message: str):
        ...
        assert any(
            expected_message in msg for msg in self.printed_messages
...
class MockWatchdog:
    ...
    def _schedule(self, handler, path, recursive=False):
        ...
        self.handlers.append({"handler": handler, "path": path, "recursive": recursive})
...
def create_mock_operation_result(
    success: bool = True, **extra_fields
) -> Dict[str, Any]:
